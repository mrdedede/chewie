...
import '../PetShop/PetShop.css'
import 'react-calendar/dist/Calendar.css'

import MockShop from '../Search-Tab/fachada-pet.png'

export default class Veterinarian extends React.Component {
  ...
  componentDidMount() {
    axios.get(`https://chewie-api.herokuapp.com/vets/${this.queryData.shopId}`, {
      headers : {
  ...
  verifyCalendar(date, calendarId) {
    axios.get(
      `https://chewie-api.herokuapp.com/${this.shopData.username}/vetservices/${calendarId}`, {
      headers : {
        'Authorization': `bearer ${this.queryData.token}`
      }
    }).then(result => {
  ...
  postTime(date, time, selectedCalendar, petName, serviceName) {
    ...
    axios.post(`https://chewie-api.herokuapp.com/appointments`, selected,  {headers : {
        'Authorization': `bearer ${this.queryData.token}`
      }
    })
    .then(result => {
